...
- sender-restricted `ses:SendEmail` permission for `login@auth.deez.run`
...
- The Lambda receives only `ses:SendEmail`, with `Resource: "*"` constrained by `ses:FromAddress == login@auth.deez.run`. This is the least-privilege form proven to work against SES in production; the broader permissions generated by linking the SST Email resource are intentionally not used.
