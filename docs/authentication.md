...
API Gateway + Lambda (SST)
    |
    | ses:SendEmail
    | only when ses:FromAddress == login@auth.deez.run
    v
AWS SES (us-east-1)
    |
    v
...
Infrastructure lives in [`infra/`](../infra/README.md). SST's Email component manages the SES identity and Cloudflare verification records; the stack also provisions the custom MAIL FROM domain, API Gateway, Lambda, throttling, and a sender-restricted `ses:SendEmail` permission for the relay Lambda.
...
- The relay Lambda receives only `ses:SendEmail`; its IAM permission uses `Resource: "*"` with `ses:FromAddress == login@auth.deez.run`, the narrow form verified to work in production.
- The broader `ses:*`, raw-email, and templated-email permissions generated by linking the SST Email resource are intentionally not granted to the relay Lambda.
